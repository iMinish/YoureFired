...
def fireAdjacent(shipHitsAI):
    ...
    for coord in reversed(shipHitAI):
        if(((coord[0]-1,coord[1]) not in rects_clickedAI) and (((coord[0]-1) >= 0) and ((coord[0]-1) <= 7)))
            ...
            return(coord[0], coord[1]-1)
    #if none of these work, we need to go to the previous position in the for loop and try again, if there's no previous position, mediumAI needs to fire again randomly
